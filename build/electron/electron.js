...
// EXTERNAL MODULE: ./node_modules/encoding/lib/encoding.js
var encoding = __webpack_require__(5769);
;// CONCATENATED MODULE: ./node_modules/electron-fetch/lib/index.es.js
...
function Body(body, {
  ...
  if (body == null) {
    ...
    body = null;
  } else if (typeof body === 'string') ; else if (body instanceof Blob) ; else if (Buffer.isBuffer(body)) ; else if (body instanceof (external_stream_default())) ; else {
    // none of the above
    // coerce to string
    body = String(body);
  }
...
function consumeBody() {
  ...
  if (!(this.body instanceof (external_stream_default()))) {
    return Promise.resolve(Buffer.alloc(0));
  } // body is stream
...
function convertBody(buffer, headers) {
  ...
  return (0,encoding/* convert */.O)(buffer, 'UTF-8', charset).toString();
}
/**
...
function clone(instance) {
  ...
  if (body instanceof (external_stream_default()) && typeof body.getBoundary !== 'function') {
    // tee instance body
    p1 = new external_stream_.PassThrough();
    p2 = new external_stream_.PassThrough();
    body.pipe(p1);
...
function writeToStream(dest, instance) {
  ...
  if (body === null) {
    ...
    body.pipe(new external_stream_.PassThrough()) // I have to put a PassThrough because somehow, FormData streams are not eaten by electron/net
    .pipe(dest);
  }
}
...
class Response {
  constructor(body = null, opts = {}) {
    ...
    this.statusText = opts.statusText || external_http_.STATUS_CODES[this.status];
...
class Request {
  constructor(input, init = {}) {
    ...
    if (!(input instanceof Request)) {
      if (input && input.href) {
        ...
        parsedURL = (0,external_url_.parse)(input.href);
      } else {
        // coerce input to a string before attempting to parse
        parsedURL = (0,external_url_.parse)(`${input}`);
      }

      input = {};
    } else {
      parsedURL = (0,external_url_.parse)(input.url);
    }
  ...
  get url() {
    return (0,external_url_.format)(this[PARSED_URL]);
  }
  /**
   * Clone this request
   *
...
if (process.versions.electron) {
  electron = __webpack_require__(2298);
}
...
function fetch(url, opts = {}) {
  ...
  return isReady.then(() => new Promise((resolve$1, reject) => {
    ...
    const send = request.useElectronNet ? electron.net.request : (options.protocol === 'https:' ? external_https_namespaceObject : external_http_).request; // http.request only support string as host header, this hack make custom host header possible
    ...
    req.on('response', res => {
      try {
        ...
        if (fetch.isRedirect(res.statusCode) && request.redirect !== 'manual') {
          ...
          request.counter++;
          resolve$1(fetch((0,external_url_.resolve)(request.url, res.headers.location), request));
          return;
        ...
        if (request.redirect === 'manual' && headers.has('location')) {
          headers.set('location', (0,external_url_.resolve)(request.url, headers.get('location')));
        } // prepare response


        let body = new external_stream_.PassThrough();
        ...
        if (!request.useElectronNet && request.method !== 'HEAD' && codings !== null && res.statusCode !== 204 && res.statusCode !== 304) {
          ...
          if (codings === 'gzip' || codings === 'x-gzip') {
            // for gzip
            body = body.pipe(external_zlib_.createGunzip());
          } else if (codings === 'deflate' || codings === 'x-deflate') {
            // for deflate
            // handle the infamous raw deflate response from old servers
            // a hack for old IIS and Apache servers
            const raw = res.pipe(new external_stream_.PassThrough());
...
var server = http_1.default.createServer(function (req, res) {
    ...
    fs_1.default.readFile(_path, function (err, data) {
        if (err) {
            ...
            res.end();
            logger_1.default.info("req ", req.url, 404, _path, suff);
            return;
        }
...
function createWindow() {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_b) {
            ...
            logger_1.default.info("=========== innerIP ".concat((0, utils_1.getInnetIP)(), " ============"));
        ...
        switch (_b.label) {
            ...
            case 1:
                ...
                electron_1.session.defaultSession.webRequest.onBeforeSendHeaders(filter, function (details, callback) { return __awaiter(void 0, void 0, void 0, function () {
                    var secchua, bv, browserVersion, ua;
                    return __generator(this, function (_a) {
...
var electron_fetch_1 = __importDefault(__webpack_require__(8958));
var fs_1 = __importDefault(__webpack_require__(7147));
var util_1 = __webpack_require__(3980);
var path_1 = __importDefault(__webpack_require__(1017));
var logger_1 = __importDefault(__webpack_require__(6645));
...
exports.check = check;
check().catch(function (err) { return logger_1.default.warn("check update error", err.stack); });
...
var filename = isWindows ? "log.log" : config_1.default.homedir("log.log");
...
function getLatestVersion() {
    ...
    files = files
        .map(function (v) { return v.replace(/^app-v/, "").replace(/\.asar$/, ""); })
        .sort(function (a, b) {
...
function getStaticResource(_path) {
    ...
    if (isDev) {
        ...
        p = path_1.default.join(StaticResourceRoot, p);
    }
    return p;
}
